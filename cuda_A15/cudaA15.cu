...
__global__ void Generate_A15(int dn11, int dn10,
	int x_sz, int y_sz, int d_solLength, Sol *d_sol0_9, ull full_mask, int *d_CNT,
	int offset_x, int end_x) {
	// TODO 将核函数改造成将整个枚举过程加入
	int i_s = blockIdx.x * blockDim.x + threadIdx.x;
	int j_s = blockIdx.y * blockDim.y + threadIdx.y;

	int i_start_idx = offset_x + i_s * x_sz;
    int i_end_idx = dMin2(offset_x + (i_s + 1) * x_sz - 1, end_x);
    
    int j_start_idx = j_s * y_sz;
    int j_end_idx = dMin2((j_s + 1) * y_sz - 1, dn10 - 1);

	tuple3 d_sed[35]; ///Num_15 = 35
	rep (i, i_start_idx, i_end_idx) {
		rep (j, j_start_idx, j_end_idx)
			if (dcompatiable13_12[i][j]) {
				...
				rep (k, 0, 59)
					if (dcompatiable13_12_11[i][j][k]) {
						rep (m, 0, 59) 
							if (dcompatiable13_12_10[i][j][m] && (dtuple11[a][b][k].first & dtuple10[c][d][m].first) == 0) {
								// printf("%d %d %d %d %d %d %d %d\n", i, j, k, m, a, b, c, d);
								int l = 0, r = d_solLength - 1, ans = r;
								ull s = dMatchings13[i] | dMatchings12[j] | dtuple11[a][b][k].first | dtuple10[c][d][m].first;
								s = full_mask ^ s;
								while (l <= r) {
									int mid = (l + r) >> 1;
									if (d_sol0_9[mid].s >= s) {
...
int main()
{
	...
	// vector<int> tmp;
	// tmp.resize(n11);
	// rep (i, 0, n11 - 1) tmp[i] = i;
	// random_shuffle(tmp.begin(), tmp.end());
	// ull tmp_array[200000][3];
	// rep (i, 0, n11 - 1) {
	// 	tmp_array[i][0] = matching13[tmp[i]][0];
	// 	tmp_array[i][1] = matching13[tmp[i]][1];
	// 	tmp_array[i][2] = Matchings13[tmp[i]];
	// } 
	// rep (i, 0, n11 - 1) {
	// 	matching13[i][0] = tmp_array[i][0];
	// 	matching13[i][1] = tmp_array[i][1];
	// 	Matchings13[i] = tmp_array[i][2];
	// }

	// tmp.resize(n10);
	// rep (i, 0, n10 - 1) tmp[i] = i;
	// random_shuffle(tmp.begin(), tmp.end());
	// rep (i, 0, n10 - 1) {
	// 	tmp_array[i][0] = matching12[tmp[i]][0];
	// 	tmp_array[i][1] = matching12[tmp[i]][1];
	// 	tmp_array[i][2] = Matchings12[tmp[i]];
	// } 
	// rep (i, 0, n10  - 1) {
	// 	matching12[i][0] = tmp_array[i][0];
	// 	matching12[i][1] = tmp_array[i][1];
	// 	Matchings12[i] = tmp_array[i][2];
	// }
    ...
    cudaMemAdvise(d_son_blocks, sizeof(pair<int, int>) * 7, cudaMemAdviseSetReadMostly, cudaCpuDeviceId);
    
    cudaMemAdvise(dcompatiable13_12, sizeof(dcompatiable13_12), cudaMemAdviseSetReadMostly, cudaCpuDeviceId);
    cudaMemAdvise(dcompatiable13_12_11, sizeof(dcompatiable13_12_11), cudaMemAdviseSetReadMostly, cudaCpuDeviceId);
    cudaMemAdvise(dcompatiable13_12_10, sizeof(dcompatiable13_12_10), cudaMemAdviseSetReadMostly, cudaCpuDeviceId);
	...
	#pragma omp parallel num_threads(num_gpus)
	{
		...
		//确定线程分配的任务数
		int chunk_size = (n11 + num_gpus - 1) / num_gpus;
		int offset_x = chunk_size * dev_id;
		int end_x = min(offset_x + chunk_size, n11) - 1;

		if (offset_x <= end_x) {
			int current_n11 = end_x - offset_x + 1;
			const dim3 GRID_SIZE(32, 32);
			const dim3 BLOCK_SIZE(32, 32);
			int x_sz = (current_n11 + 1023) / 1024, y_sz = (n10 + 1023) / 1024;

			// cout << x_sz << ' ' << y_sz << ' ' << sol0_9.size() << endl;

			Generate_A15<<<GRID_SIZE, BLOCK_SIZE>>>(n11, n10, x_sz, y_sz, sol0_9.size(), d_sol0_9, full_mask,
				d_CNT, offset_x, end_x);
			...
			cudaMemcpy(&host_cnt[dev_id], d_CNT, sizeof(int), cudaMemcpyDeviceToHost);
		}
		cudaFree(d_sol0_9);
        cudaFree(d_CNT);
	}
	...
	for (int i = 0; i < num_gpus; i++) {
        total_cnt += host_cnt[i];
		// printf("The CNT is: %lld\n", host_cnt[i]);
    }
